...
static uint8_t RC[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,/* 0x6c, 0xd8, 0xab, 0x4d*/};
...
int main()
{
	...
	uint8_t pad_len = sizeof(plaintext)/sizeof(uint8_t);
    ...
    if(ciphertext[15]>0x00&&ciphertext[15]<16)   rem_padding = (16 - ciphertext[15]);
    else                    rem_padding = AES_BLOCK_SIZE;



    for (i = 0; i < rem_padding; i++) {
		printf("%2x ", ciphertext[i]);
	}
